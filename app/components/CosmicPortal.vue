...
function addPortalShader(material: any) {
  ...
  material.onBeforeCompile = (shader: any) => {
    ...
    shader.uniforms.portalSpeed = { value: 5.0 }
...
function createPortalCore() {
  // Replace SphereGeometry with TetrahedronGeometry
  const geo = new THREE.TetrahedronGeometry(0.8) // 0.8 is the radius, adjust as needed
...
function createFloatingCrystals() {
  ...
  for (let i = 0; i < params.value.crystalCount; i++) {
    const geo = new THREE.TetrahedronGeometry(0.3 + Math.random() * 0.9)
    const mat = new THREE.MeshPhysicalMaterial({
...
function createDimensionalStreams() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j <= segments; j++) {
      const t = j / segments
      const angle = t * Math.PI * 22 + i * Math.PI * 1.15
      const radius = 3 * 1 + Math.sin(t * Math.PI * 6) * 2.5
...
function createSpaceDistortion() {
  const geo = new THREE.TetrahedronGeometry(12)
  const mat = new THREE.ShaderMaterial({
    uniforms: {
      time: { value: 0 },
      dimensionShift: { value: 0 },
...
function animate() {
  ...
  if (camera.position.z < 10) {
    ...
    camera.position.z += 0.05 // Adjust speed as needed
    // camera.position.y += 0.05 // Adjust speed as needed
    controls.update()
  }
